            ...
            <div className="flex gap-1">
              {["day", "week", "month"].map((p) => (
                <button
                  key={p}
                  onClick={() => setPeriod(p)}
                  className={`text-[11px] font-mono px-2.5 py-1 rounded-card capitalize transition-colors ${period === p ? "bg-violet-soft text-violet font-semibold" : "text-ink-faint hover:text-ink"}`}
                >
                  {p}
                </button>
              ))}
